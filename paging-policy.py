...
if options.solve == False:
    ...
    if policy == 'FIFO':
        ...
        for i in tmp:
            intI = int(i)
            addrList.append([intI,0,0])  #[address, frequency, index]
    ...
    if policy == 'LFU':
        for nStr in addrList:
            ...
            victim = -1
            if idx == -1:
                # in case of miss, replace?
                if count == cachesize:
                    # replacement needed
                    # find least frequently used and replace
                    tmp = sorted(memory, key = operator.itemgetter(1))  #frequency 순으로 정렬
                    victimIndex = tmp[0][2]
                    victim = memory.pop(victimIndex)
                    for i in range(0, len(memory)):
                        memory[i][2] = i
                else:
                    # no replacement
                ...
                if victim != -1:
                    # when replaced, if still exist -> assert
                    assert(victim not in memory)
            if notrace == False:
                print('Access: %d  %s %s -> %12s <- %s Replaced:%s [Hits:%d Misses:%d]' % (n[0], hfunc(idx), leftStr, memory, riteStr, vfunc(victim), hits, miss))
